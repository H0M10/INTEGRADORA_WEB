            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-sm font-medium text-gray-700">Código de Vinculación *</label>
                  {modalMode === 'create' && (
                    <button
                      type="button"
                      onClick={async () => {
                        try {
                          const { device_code } = await devicesService.generateCode()
                          setFormData({ ...formData, code: device_code })
                ...
                <input type="text" value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '') })} maxLength={20} className={`w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono tracking-wider ${formErrors.code ? 'border-red-500' : 'border-gray-200'}`} placeholder="NOVA001" disabled={modalMode === 'edit'} />
